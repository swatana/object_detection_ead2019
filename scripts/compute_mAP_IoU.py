...
predictfolder = res_dir+"/predictions/"
result_graph_dir = os.path.join(res_dir, 'graphs')
resultsfolder = res_dir+"/mAP_IOU/"

jsonFileName = res_dir+"/mAP_IOU/metrics.json"

os.makedirs(result_graph_dir, exist_ok=True)
...
def plot_and_calc_ap(y_true, y_score, class_name, result_graph_dir):
    fpr, tpr, thresholds = metrics.roc_curve(y_true, y_score)
    precision, recall, thresholds = metrics.precision_recall_curve(y_true, y_score) # tpr is same to recall actually
    auc = metrics.auc(fpr, tpr)
    ap = metrics.average_precision_score(y_true, y_score, average='samples')
    # print(class_name, ap)
    ...
    for _tpr, _fpr in zip(tpr, fpr):
      if _tpr - _fpr > max_cutoff:
        max_cutoff = _tpr - _fpr
        thre_tpr = _tpr
        thre_fpr = _fpr

    plt.figure()
    os.makedirs(os.path.join(result_graph_dir,class_name), exist_ok=True)
    plt.xlim(-0.1, 1.1)
    plt.ylim(-0.1, 1.1)
    plt.fill_between(fpr, 0, tpr, color='y', alpha=0.5)
    plt.plot(fpr, tpr, 'b')	
    plt.plot(thre_fpr, thre_tpr, color='r',	
             marker='.', markersize=20, alpha=0.7)
    plt.title('ROC curve ' + class_name)
    plt.xlabel('False Positive Rate')
    plt.ylabel('True Positive Rate')
    plt.grid(True)
    plt.savefig(os.path.join(result_graph_dir,class_name,'roc.png'))
    plt.close()
    plt.figure()
    ...
    plt.fill_between(recall, 0, precision, color='y', alpha=0.5)
    plt.title('PR curve ' + class_name)
    plt.xlabel('recall')
    plt.ylabel('precision')
    plt.grid(True)
    plt.savefig(os.path.join(result_graph_dir,class_name,'pr.png'))
    plt.close()

    return tpr, fpr, precision, recall, auc, ap
...
#    error_msg += "(You can avoid this error message by running extra/intersect-gt-and-pred.py)"
  ...
  for line in lines_list:
    try:
      _line = line.split()
      class_name = " ".join(_line[:-4])
      left, top, right, bottom = _line[-4:]
    except ValueError:
...
#        error_msg += "(You can avoid this error message by running extra/intersect-gt-and-pred.py)"
    ...
    if lines==[]:
        ...
        for line in lines:
          try:
            _line = line.split()
            tmp_class_name = " ".join(_line[:-5])
            confidence, left, top, right, bottom = _line[-5:]
          except ValueError:
            error_msg = "Error: File " + txt_file + " in the wrong format.\n"
...
with open(os.path.join(results_files_path, "results.txt"), 'w') as results_file:
  ...
  for class_index, class_name in enumerate(gt_classes):
    ...
    for idx, prediction in enumerate(predictions_data):
      ...
      is_tp = ovmax > MINOVERLAP and not gt_match['used']
      result.append((float(prediction['confidence']), is_tp))
    ...
    y_true = [1 if r[1] else 0 for r in result]
    y_score = [r[0] for r in result]

     # save ROC curve and PR curve
    if len(y_true) is not 0:
      tpr, fpr, precision, recall, auc, ap = plot_and_calc_ap(y_true, y_score, class_name, result_graph_dir)
    ...
    mean_iou = np.mean(iou) if len(iou) > 0 else 0
    text = "{0:.2f}% {1:.2f}%".format(ap*100, mean_iou*100) + " = " + class_name + " AP  IoU"
    print (text)
    """
    ...
    iou_dictionary[class_name] = mean_iou
    sum_iou += mean_iou
